...
async function getInfo(){
    ...
    for(const links of linksList){       
        ...
        const teamAYellowCards = await page.$$eval('div > .a > div > .yellow_card', yellowCardsA=> yellowCardsA.map(cardsA => cardsA));
        const teamBYellowCards = await page.$$eval('div > .b > div > .yellow_card', yellowCardsB=> yellowCardsB.map(cardsB => cardsB));
        ...
        const match = {
            ...
            teamA:{
                name:matchString[0],
                yellowCard: teamAYellowCards.length,
            ...
            teamB:{
                name:matchString[2],      
                yellowCard: teamBYellowCards.length,
